...
$cmd='sudo ogr2ogr -f "GPKG" /var/www/html/nextcloud/data/'.$row[3].'/files/_qfield/observations.gpkg PG:"dbname=geonature host=192.168.1.244 port=5432 user=postgres password=conservatoire" -nln \'nomenclature\' -sql "SELECT * FROM _qfield.nomenclature;" -overwrite -update 2>&1';
echo '</br>'.$cmd.'</br>';
exec($cmd, $output);
	if (!$output) {
