...
$select = pg_prepare($dbconn_geo, "sql_select", "select courriel, gn_user_name, nom_ad, uuid_nx from $nx_users where observations_gpkg <> 'ø' and (obs_faune > 0 ) ;");
pg_prepare($dbconn_geo, "sql_import_occtax", "select sandbox.import_faune_();");
//$delete = pg_prepare($dbconn_geo, "sql_delete", "delete from sandbox.obs_faune_tmp ;");
$personne = pg_execute($dbconn_geo, "sql_select",array()) or die ( pg_last_error());


$tables_faune = array(' obs_faune ', ' obs_faune_polygone '); //, ' obs_faune_ligne '


while($row = pg_fetch_row($personne))
{
  echo '</br>----------------------------------------------------------</br>';
  echo 'Import des données faune pour l\'utilisateur : <span class="text-danger">'.$row[2].'</span> ('.$row[0].' - '.$row[3].')</br>';
  $observations_gpkg = '/var/www/html/nextcloud/data/'.$row[3].'/files/_qfield/observations.gpkg';
  if (file_exists($observations_gpkg)) {
    foreach ($tables_faune as $table) {
        $cmd_='ogr2ogr -f PostgreSQL "PG:user='.$LOGIN_geonature.' host='.$DBHOST_geonature.' dbname='.$DBNAME_geonature.' password='.$PASS_geonature.'" /var/www/html/nextcloud/data/'.$row[3].'/files/_qfield/observations.gpkg -nln sandbox.obs_faune -append -sql "SELECT *, \''.$row[0].'\' as courriel, \''.$row[3].'\' as uuid_nx from '.$table.'  where date_import is null" 2>&1';
        $output_=[];
        $return_var=0;
        ...
        if ($return_var == 0) {
            ...
            if ($results_write_gpkg) {
                //echo '</br>Données faune ( '.$table.' ) importées avec succès ! </br>';
                echo $db->changes();
    ...
    //intégration des données de sandbox.obs_faune dans geonature --> occ_tax
    $out = pg_execute($dbconn_geo, "sql_import_occtax",array()) or die ( pg_last_error());
    if ($out) {
        echo 'Import des données faune dans Geonature réussi !</br>';
    } else {
