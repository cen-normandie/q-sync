...
while($row = pg_fetch_row($personne))
{
  ...
  if (file_exists($observations_gpkg)) {
		//Lit toutes les obs faune non importées depuis les geopackage et les envoies dans postgres sandbox.faune
		$cmd='ogr2ogr -f PostgreSQL "PG:user='.$LOGIN_geonature.' host='.$DBHOST_geonature.' dbname='.$DBNAME_geonature.' password='.$PASS_geonature.'" /var/www/html/nextcloud/data/'.$row[3].'/files/_qfield/observations.gpkg -nln '.$point_faune.' -append -sql "SELECT *, \''.$row[1].'\' as courriel, \''.$row[3].'\' as uuid_nx from '.$faune.' where date_import is null" 2>&1';
		exec($cmd, $output);
		if ($output != 2) {
			...
			if ($out) {
				//Pour chaque observation importée, on va mettre à jour la date d'import dans le geopackage
				...
				while($row_ = pg_fetch_row($to_up))
					{
						...
						if ($results_write_gpkg) {
							//lance un scan des fichiers pour mettre à jour les geopackages de nextcloud
