...
echo "INITIALISATION";
echo "</br>";
echo "sqlite3 /var/www/html/q-sync/_qfield_skeleton/". $gpkg ." '.dump ".$table."' > /var/www/html/q-sync/_qfield_skeleton/dumps/".$table.".sql";
...
echo "</br>Export depuis ".$gpkg." SKELETON with status $retval and output:";
echo "</br>";
echo '</br>#########';
...
while($row = pg_fetch_row($personne))
{
  ...
  if (file_exists($path_gpkg)) {
    //echo '</br>' .$row[0]. ' - ' . date('Y-m-d', filemtime($path_gpkg)) . '</br>';
    echo '</br>#########' .$row[0].'</br>';
    exec("sqlite3 ".$path_gpkg." 'DROP TABLE ".$_POST['table_name'].";'", $output, $retval);
    echo "Suppression de la table ".$_POST['table_name']." dans ".$path_gpkg." avec status $retval";
    echo '</br>';
    exec("sqlite3 ".$path_gpkg."  < /var/www/html/q-sync/_qfield_skeleton/dumps/".$table.".sql", $output, $retval);
    echo "Importation de la table ".$table." dans ".$path_gpkg." avec status $retval";
    echo '</br>';
  }
}
