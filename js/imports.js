...
const dtQSync_n2k =$('#QSync_n2k').DataTable({
    "language": {
        "paginate": {
            "previous": "Préc.",
            "next": "Suiv."
...
console.log('load n2k');
    ...
    $.ajax({
        ...
        success  : function(data) {
            qsync_liste = data ;
            data_json =[{
                n2k_polygone_a_integrer: 0,
                n2k_polygone_en_attente: 0,
                n2k_ligne_a_integrer: 0,
                n2k_ligne_en_attente: 0,
                n2k_point_a_integrer: 0,
                n2k_point_en_attente: 0,
                color: '#007bff'
            }];
            console.log(data);
            for (const ele in qsync_liste) {
                ...
                data_json[0].n2k_ligne_en_attente += parseInt(qsync_liste[ele].n2k_previ_ligne.split(' / ')[1]);
                data_json[0].n2k_point_a_integrer += parseInt(qsync_liste[ele].n2k_previ_point.split(' / ')[0]);
                data_json[0].n2k_point_en_attente += parseInt(qsync_liste[ele].n2k_previ_point.split(' / ')[1]);
                let rowNode = dtQSync_n2k.row.add( [
                    qsync_liste[ele].uuid,
                    qsync_liste[ele].personne, 
                    qsync_liste[ele].n2k_previ_polygone,
                    qsync_liste[ele].n2k_previ_ligne,
                    qsync_liste[ele].n2k_previ_point,
                    qsync_liste[ele].update
                ] ).node().id = qsync_liste[ele].uuid;
            }
            dtQSync_n2k.draw();
